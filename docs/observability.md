...
- Optional debugger UI:
  ...
  **Dev mode (always-available dev timeline).** For the inner development loop,
  ...
  process. The UI adds a **live session selector** (switch among concurrently
  running browser/websocket/twilio sessions), **budget overlays** fed by the
  shared budget report (`GET /api/budgets`), and a **promote-turn-to-test**
  action (`POST /api/dev/promote`) that routes through the redact-by-default
  promotion path.
