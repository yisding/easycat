...
| `LocalTransportConfig.output_preroll_frames` | `3` | Before each local-speaker utterance (and after barge-in), waits for this many output frames to queue. At the default 20 ms frame size, normal multi-frame TTS chunks usually reduce the cost to one callback period; very small chunks can pay up to about 60 ms. | Set to `0` to disable when the TTS stream is already well paced, or lower it to trade underrun protection for faster first sound. Tune together with `frame_duration_ms`. |
