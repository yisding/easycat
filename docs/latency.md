...
These are the defaults that *add waiting time* on the response path or protect
the audio context around a delayed decision. Each value below is asserted
against the code by a guard test
...
| `TurnManagerConfig.punctuated_end_of_turn_silence_ms` | `200` | Shortens the fixed timer only after STT finalizes text ending in terminal punctuation. A trailing run of two or more full stops, or a Unicode ellipsis, reads as trailing off rather than finishing, so it keeps the full timer. | Set to `None` to disable. Smart-turn incomplete/error decisions retain the full fallback timer, so punctuation never overrides a semantic incomplete verdict. |
...
| `LocalTransportConfig.output_preroll_frames` | `3` | At local-speaker session start and after barge-in, waits for this many output frames to queue. Ordinary queue underruns stay primed so a streaming utterance does not repeatedly pay this delay. At the default 20 ms frame size, normal multi-frame TTS chunks usually reduce the initial cost to one callback period; very small chunks can pay up to about 60 ms. | Set to `0` to disable when the TTS stream is already well paced, or lower it to trade underrun protection for faster first sound. Tune together with `frame_duration_ms`; for a bounded output queue this value cannot exceed `max_pending_out_chunks`. |
| `TurnManagerConfig.pre_roll_ms` | `450` | Does not add a wait; retains audio from before confirmed VAD start so the speech-confirmation gate does not clip the utterance onset. | Keep this at least `VADConfig.min_speech_duration_ms + 150`. `EasyConfig` warns when a typed VAD config and pre-roll violate that margin; raise both together when tuning out echo, coughs, or background noise. |
...
| `VADConfig.min_speech_duration_ms` | `250` | Speech must persist this long before the VAD reports start-of-speech. | Delays turn start and barge-in detection slightly. This gate is the only debounce on barge-in — a confirmed start-of-speech during bot playback cancels in-flight TTS/agent work — so lowering it increases false triggers from echo, coughs, and background noise. If you raise it, also keep `TurnManagerConfig.pre_roll_ms` at least 150 ms higher so the first word stays buffered. |
...
[`transports/local.py`](../src/easycat/transports/local.py),
