...
| `TurnManagerConfig.punctuated_end_of_turn_silence_ms` | `50` | Shortens the fixed timer only after STT finalizes text ending in terminal punctuation, targeting about 100 ms of total silence with the default VAD. | Set to `None` to disable. Smart-turn incomplete/error decisions retain the full fallback timer, so punctuation never overrides a semantic incomplete verdict. |
