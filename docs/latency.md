...
| `TurnManagerConfig.punctuated_end_of_turn_silence_ms` | `250` | Shortens the fixed timer only after STT finalizes text ending in terminal punctuation. | Set to `None` to disable. Smart-turn incomplete/error decisions retain the full fallback timer, so punctuation never overrides a semantic incomplete verdict. |
