...
|---|---|---|
| No mic or API keys | [Hardware-free checkpoint spine](./offline_spine.py), [`11-journal`](./11-journal/), or [`12-evals-and-latency`](./12-evals-and-latency/) | The spine reaches every chapter without credentials; chapters 11–12 use checked-in bundles, and chapter 12's `llm_judge.py` is the only optional live-key script. |
