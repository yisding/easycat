...
both connection contexts. It finishes with a two-session `stop_all()`
sweep where one `stop()` raises. Which guarantees belong to the manager,
and which cleanup remains the session's responsibility?
