...
No API keys, no network, no microphone. `custom_vad.py`'s `register()` is
confirmed to make the custom VAD selectable, `agent.py`'s wiring is asserted
...
`test_custom_vad.py` for the provider's own conformance suite and AGENTS.md
for the live eval ladder.
...
import pytest
from easycat import available_vad_providers, create_vad
...
from custom_vad import EnergyVAD, register
...
def test_register_makes_the_custom_vad_selectable() -> None:
    register()
