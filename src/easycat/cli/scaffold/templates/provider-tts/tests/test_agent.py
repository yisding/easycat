...
No API keys, no network, no microphone. `custom_tts.py`'s `register()` is
confirmed to make the custom TTS selectable, `agent.py`'s wiring is asserted
...
`test_custom_tts.py` for the provider's own offline contract suite and
...
from custom_tts import ToneTTS, register
...
def test_register_makes_the_custom_tts_selectable() -> None:
    register()
    assert "tone" in available_tts_providers()
    assert isinstance(create_tts_provider(TTSProviderConfig(provider="tone")), ToneTTS)
