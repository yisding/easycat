...
`agent.py`. Keep `agent.py` import-safe: `make_agent()` builds things,
and `server.py` wires it into the call server. Plain, SDK-free helpers
belong in `tools.py`.
...
uv run ruff check agent.py server.py tools.py       # Lint
uv run ruff check --fix agent.py server.py tools.py # Auto-fix lint findings
