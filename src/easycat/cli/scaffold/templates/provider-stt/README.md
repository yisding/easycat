...
External EasyCat STT package: typed config, structural provider, named
registration, lazy package entry point, capability metadata, and offline
behavioral contracts. `ScriptedSTT` is deterministic so the starter works
without provider traffic; replace its marked backend seam with your SDK.
...
Set `OPENAI_API_KEY` for the demo's agent and default TTS. The custom STT is
offline. When you add a live backend, add its key to `.env.example`, its
config dataclass, and `register_stt_provider(env_var=...)` together.

## Run

The local demo selects the public `stt="scripted"` shortcut:
...
```bash
uv run pytest test_custom_stt.py
```

The installable `STTProviderContractSuite` checks stream reuse, commit
acceptance, normalized final events, idempotent shutdown, and all four
...
`uv run ruff check --fix agent.py custom_stt.py test_custom_stt.py`, then run
...
- Replace the `LIVE TODO` seam with an async SDK client. Add
  ...
  `capabilities` accurate (only add `native_endpointing` if finals own turns).
- Add a separate suite marked `integration_live`, `provider_custom`, and
  `surface_stt`, with `live = True` and `credential_env_var`; never make the
  offline contract class use the network.
- Publish the `easycat.stt_providers` entry point from `pyproject.toml`; users
