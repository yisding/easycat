...
`agent.py`. Keep `agent.py` import-safe: `make_agent()` / `make_app()`
...
`tests/test_agent.py` runs offline: `tools.py` runs for real,
`agent.py`'s wiring (name, instructions, tool list, `VoiceApp` fields)
is asserted as built, and `ScriptedReasoning` stands in for the model
while EasyCat's real turn machinery (the send_text path, the audio
pipeline, journal, and latency metrics) runs end to end. A green run means
the app is wired and the pipeline is healthy; it
