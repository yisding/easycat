...
- and the UI exposes budget overlays and a promote-turn-to-test action.
...
def session_budgets(session: Any) -> tuple[list[Any], list[Any]]:
    """Resolve ``(latency_budgets, cost_budgets)`` configured for *session*.

    Reads the session's :class:`~easycat.config.EasyConfig` observability block
    and lifts ``latency_budget`` / ``max_session_cost_usd`` into the shared
    budget value objects (:class:`~easycat.budgets.LatencyBudget` /
    :class:`~easycat.budgets.CostBudget`). Degrades to empty lists when no
    config or no budgets are attached so ``/api/budgets`` always renders.
    """
    ...
    raw_latency = getattr(observability, "latency_budget", ()) or ()
    if isinstance(raw_latency, LatencyBudget):
        latency_budgets = [raw_latency]
    else:
        latency_budgets = [b for b in raw_latency if isinstance(b, LatencyBudget)]

    cost_budgets: list[CostBudget] = []
    max_cost = getattr(observability, "max_session_cost_usd", None)
    if max_cost is not None:
        cost_budgets.append(CostBudget(max_session_usd=float(max_cost), action="warn"))

    return latency_budgets, cost_budgets
