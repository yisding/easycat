...
**Provider subpackages** (`stt/`, `tts/`, `vad/`, `transports/`, `telephony/`): one provider per file, each implementing the corresponding Protocol. Base classes (`STTBase`, `TTSBase`, `ServerTransportBase`) provide shared plumbing; `AudioQueueMixin`, `ServerTransportBase`, and `TransportDegraded` are re-exported from `easycat.transports` for out-of-tree transports (see `docs/extending/`).
