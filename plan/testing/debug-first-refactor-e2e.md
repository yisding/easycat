...
> **Status (2026-05-21): historical strategy, partially implemented.**
> The original five plans below are now represented by concrete tests under
> `tests/e2e/`; the current suite also adds Plan 6 full-stack NR/AEC coverage
> and Plan 7 latency benchmarking. Keep this document as the design rationale
> for the E2E suite, not as the complete current test index.

Five original end-to-end test plans that push real voice bytes through the
full EasyCat pipeline (transport -> VAD -> STT -> agent -> TTS -> transport)
and validate the debug-first contract under stress and at the edges. Every
plan uses real audio in and real audio out — no scripted STT transcripts, no
...
- `tests/e2e/test_plan_1_baseline_voice_roundtrip.py`
- `tests/e2e/test_plan_2_sustained_stress.py`
- `tests/e2e/test_plan_3_adversarial_audio.py`
- `tests/e2e/test_plan_4_interruption_matrix.py`
- `tests/e2e/test_plan_5_record_replay_fidelity.py`
- `tests/e2e/test_plan_6_full_stack_with_nr_and_aec.py`
