...
| AC2.6d | New test `test_pydantic_ai_graph_mode_missing_event_handler_warning` — runs a graph whose nodes deliberately omit `event_stream_handler=...` when calling agents, asserts exactly one warning journal record naming the convention, asserts the turn completes successfully and `workflow_node` records are still present. |
