...
> **Status: historical acceptance record.** The runtime journal, artifact
> store, bundle, replay, and debugger primitives exist in the current tree, and
> the old legacy observability modules are absent. Keep this file for rationale
> and acceptance history; verify new work against
...
- Full `RedactionPolicy` write filter with per-field strategies,
  ...
  export-time redaction pass (peripheral — `../peripherals/peripheral-redaction.md`).
...
- [x] Expose a single extension point (`apply_write_filter(record)`)
  ...
  `../peripherals/peripheral-redaction.md` plugs into later to layer a full
...
- [x] **AC1.7** Config and Environment Safety Default is enforced.
  ...
  out of scope here and lives in `../peripherals/peripheral-redaction.md`.
...
- **Safe default allowlist gaps**: the hard-coded config/env allowlist
  ...
  `../peripherals/peripheral-redaction.md`; WS1's guardrail is intentionally
  minimal.
