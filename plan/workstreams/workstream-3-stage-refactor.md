...
> **Status: partially stale historical record.** Session decomposition has
> landed through `AudioRouter`, `STTCommitter`, `TTSScheduler`,
...
> 1,773 lines in the 2026-05-21 snapshot, so the historical `<500` target was
> not met.
...
Historical target: decompose `src/easycat/session/_session.py` into stage +
context + controller types so stage execution is the main orchestration model,
each stage boundary is journaled with `state_before`/`state_after` snapshots,
and upstream control signals (interruption, cancel, pause, backpressure) are
recorded per-stage.
...
- `WarmupStage` (peripheral — in `../peripherals/peripheral-observability-and-cost.md`)
...
- [x] `RunContext` stores a safe config snapshot via the WS1 hard-coded
  ...
  A full `RedactionPolicy` lands in `../peripherals/peripheral-redaction.md` and
...
- [x] `StageStateSnapshot` values flow through the WS1 T1.5
  ...
  no-op for non-sensitive fields in WS1; `../peripherals/peripheral-redaction.md`
