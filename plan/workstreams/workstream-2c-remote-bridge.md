...
> **Status: historical acceptance record.** `RemoteResponsesAPIBridge` exists
> under `easycat.integrations.agents.responses_api` with mock-server tests and
> current package exports. Use this file for rationale, not as proof that every
> detailed acceptance item still matches source names.
