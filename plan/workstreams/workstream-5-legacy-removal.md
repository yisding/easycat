...
> **Status: historical acceptance record.** The old root legacy files named in
> this workstream are absent from the current tree. The active
> `easycat.integrations.agents._agent_runner.AgentRunner` is a current bridge
> helper and is not the removed legacy `src/easycat/agent_runner.py` module.
...
- [x] Update imports to use journal-derived tracing (peripheral
  OTel exporter in `../peripherals/peripheral-observability-and-cost.md`
