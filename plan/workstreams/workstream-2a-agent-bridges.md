...
> **Status: historical acceptance record.** The bridge package now includes
> OpenAI Agents, PydanticAI, generic workflow, Remote Responses API,
> LangChain, LangGraph, and Llama Agents. `AgentRunner` remains active under
> `easycat.integrations.agents._agent_runner`; do not confuse it with the
> removed legacy root `src/easycat/agent_runner.py`.
...
- Framework state snapshots routed through the WS1 safe-default
  ...
  in `../peripherals/peripheral-redaction.md` without bridge changes)
...
- [x] `FrameworkStateSnapshot` must be JSON-safe, honor the WS1
  ...
  `../peripherals/peripheral-redaction.md` later layers a full `RedactionPolicy`
...
- [x] Create shared event-translator module
  ...
  `../peripherals/peripheral-langchain-langgraph-bridge.md`) follows the same
...
- [x] **AC2.18** `FrameworkStateSnapshot` values for every bridge
  ...
  here and lives in `../peripherals/peripheral-redaction.md`.
