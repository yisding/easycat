...
- Dev-only banner stamped on every exported bundle (the banner text
  ...
  `../peripherals/peripheral-redaction.md` later)
...
- [x] Design decisions covering:
  ...
  - Dev-only banner text read from WS1 `safe_defaults.py` and the
    hook `../peripherals/peripheral-redaction.md` uses to upgrade the banner and
...
- [x] Define `RunBundle` dataclass:
  ...
  - `sharing_banner: str` — dev-only banner text read from WS1
    `safe_defaults.py`; `../peripherals/peripheral-redaction.md` later upgrades
...
- [x] A future `redaction=` kwarg is reserved for
  `../peripherals/peripheral-redaction.md`, which adds an export-time second pass
...
- [x] **AC4.11** Bundle safe-snapshot rules ensure the bundle
  ...
  is reserved for `../peripherals/peripheral-redaction.md` and not in scope here.
...
- `../peripherals/peripheral-eval-and-debugger-ui.md` adds `forked_replay` as a
  fourth fidelity class and builds the Simulator + Judge on top of
  the `load_bundle()` fixture helper
- `../peripherals/peripheral-cli.md` wraps `export_debug_bundle` and the bundle
  discovery functions as `easycat bundles export` and
  `easycat bundles list` CLI commands
- `../peripherals/peripheral-observability-and-cost.md`'s `CostRecord` is included
