...
| [onramp-zen-dx-plan.md](onramp-zen-dx-plan.md) | Mostly landed implementation record | Preserves the verifier-backed DX onramp decisions that shaped the canonical hello-world, error-teaching, lifecycle, and scaffold signposting work; keep here instead of a separate top-level DX tree. |
...
| [peripheral-observability-and-cost.md](peripheral-observability-and-cost.md) | Mostly planned | Debugger cost endpoint degrades to zero and reports `max_session_cost_usd` budget status when cost records exist; session journals emit budget warning/exceeded records from explicit cost entries and stop sessions after exceeded budgets; stage-record latency-budget tags, `total_ms` runtime alerts, and startup `warmup()` hook execution exist. Real provider `CostRecord` emission/pricing, OTel export, first-token/audio budgets, and provider-native warmup coverage remain planned. |
