...
> **Status (2026-05-21): partially landed.** Cartesia STT/TTS, Deepgram
> Flux parsing/plumbing, and bundled Smart Turn v3.2 ONNX support exist in
> current code. Backchannel filtering, provider capability reports, and
> making Smart Turn a top-level default-on promotion remain planned.
>
> **This is a peripheral initiative.** It is not essential to the
> debug-first thesis in `../roadmap/essential-debug-first-runtime.md`. It is what
...
`../roadmap/essential-debug-first-runtime.md`).
...
top-level `smart_turn=True` with a single `smart_turn_sensitivity` knob.
Enable by default where the ONNX runtime is available.
...
- **OpenAI Realtime and Gemini Live** (2026): voice-to-voice /
  ...
  `../roadmap/essential-debug-first-runtime.md`. Users who need realtime use the
