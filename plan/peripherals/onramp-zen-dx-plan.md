...
confronts a newcomer with 94 flat alphabetical names where the two they need are buried at
...
| #6 Sparse is better than dense | 2 | 3 | `__all__` stays the curated 94 (the snapshot is a tested contract), but the package `__doc__` now leads with a "start here" and `bot.py` gives an in-editor ladder, so the *effective* discovery surface is sparse even though the name list is not shrunk. |
...
**Files touched.** `src/easycat/config/easy.py`; `tests/config/test_debug_errors.py` (the empty-key assertions
that still expect a `ValueError` stay green because we keep the per-provider `ValueError`);
`tests/examples/` only needs review if the no-key stderr assertion is exercised with a
genuinely-missing key.

**Verifier verdict.** Landed and guarded by
`tests/config/test_debug_errors.py::test_missing_openai_key_with_no_stt_tts_raises_e203`. The **original
...
**Verifier verdict.** Landed and guarded by
`tests/config/test_agent_adaptation.py::test_create_session_rejects_bogus_agent`,
`tests/config/test_agent_adaptation.py::test_create_session_rejects_sync_run_agent`, and
`tests/config/test_agent_adaptation.py::test_create_session_skips_agent_check_when_wrap_agent_false`. The
...
`tests/config/test_text_session_config.py`, and would force newcomers onto `TextSessionConfig` (not even in
...
**Files touched.** `README.md`; `tests/install/test_install_guidance.py`.
...
2. `src/easycat/config/easy.py` —
   ...
   - **leave** the per-provider empty-key `ValueError` (with `_provider_display_name`) untouched —
     ...
     leverage.
3. `tests/` —
   - `tests/config/test_debug_errors.py` has a case asserting `EASYCAT_E203` (an `EasyCatError`) is raised
   ...
   - `tests/examples/` — review only if a no-key stderr assertion is exercised.
...
- **The tested 94-name `__all__` contract.** We do **not** shrink it (verifier: drop). It is a
