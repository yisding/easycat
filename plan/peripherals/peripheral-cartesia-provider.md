...
> Keep this file as the historical implementation plan and provider API
> reference, not as an active checklist.
>
> **This is a peripheral initiative.** It is not essential to the
> debug-first thesis in `../roadmap/essential-debug-first-runtime.md`. It adds
...
>   Turbo as the 2026 TTS latency bar. Cartesia adapter coverage now lives
>   in the current provider modules; keep that doc's competitive context in
...
- **`add_timestamps: true` by default in TTS.** Cheap, and word
  ...
  once that lands (see `../roadmap/essential-debug-first-runtime.md`). Gate it
...
4. Keep `peripheral-provider-ecosystem.md` aligned with the landed
   Cartesia adapter status.
