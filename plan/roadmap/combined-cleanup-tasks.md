...
- `src/easycat/session/_session.py` is now roughly 1,440 lines, not 2,961.
...
- `src/easycat/__init__.py` is now 161 lines, not 578.
- The top-level lazy export list is now 92 symbols, not 195.
...
- Done: timeout wrapper tests model non-completing providers with
...
- Done: bind-close-reuse free-port helpers were removed from shared test
  helpers; migrated socket tests now use `unused_tcp_port_factory` or bind the
  server first and read back the live port.
...
This sequence is the April audit order. Items 2, 3, 5, 6, and parts of 1, 4,
