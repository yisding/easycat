...
> authoritative. Use `current-code-status.md` for current source-tree status.
...
> essential to that thesis — it lives in the peripheral
...
>
> - `../peripherals/peripheral-deployment.md` — per-platform runbooks for the
...
> - `../peripherals/peripheral-dx-onboarding.md` — library DX: line budgets,
...
> - `../peripherals/peripheral-cli.md` — `easycat` CLI focused on scaffolding
...
> - `../peripherals/peripheral-observability-and-cost.md` — `CostRecord` with pricing
>   source, budget alerts, `JournalToOTelExporter` with `gen_ai.*`
>   semconv, Latency Budget targets, `WarmupStage`.
> - `../peripherals/peripheral-eval-and-debugger-ui.md` — `easycat.testing`,
...
- **Rate limiting and quota.** Per-session cost budgets are
  addressed in `../peripherals/peripheral-observability-and-cost.md`. Global
...
- **CI/CD harness for bundles.** WS4 ships `load_bundle()` as a
  ...
  `../peripherals/peripheral-eval-and-debugger-ui.md` and
  `../peripherals/peripheral-dx-onboarding.md`, and is not required
  for the debug-first thesis.
...
`../peripherals/peripheral-deployment.md`; this section pins the constraints and
...
any other code. `../peripherals/peripheral-deployment.md` covers the tuning
...
`../workstreams/workstream-2c-remote-bridge.md` for the operational plan.
...
- **LangChain and LangGraph bridges** — deferred, not excluded.
  ...
  `OpenAIAgentsBridge`. See `../peripherals/peripheral-langchain-langgraph-bridge.md`
...
- safe config snapshot (hard-coded allowlist — see "Config and
  ...
  in `../peripherals/peripheral-redaction.md`)
...
`RedactionPolicy` from `../peripherals/peripheral-redaction.md` ships,
...
`../peripherals/peripheral-redaction.md`:
...
`../peripherals/peripheral-redaction.md` lands. Essential-plan bundles
...
- **Dev-only banner**: every essential-plan bundle carries a banner
  ...
  once `RedactionPolicy` from `../peripherals/peripheral-redaction.md`
  ships.
...
- dev-only banner on every essential-plan bundle until the peripheral
  `RedactionPolicy` ships (see `../peripherals/peripheral-redaction.md`)
...
in `../peripherals/peripheral-redaction.md`, not in this plan.
...
See `../workstreams/workstream-1-journal-foundation.md`.
...
write filter lands later in `../peripherals/peripheral-redaction.md`.
...
See `../workstreams/workstream-2a-agent-bridges.md`.
...
See `../workstreams/workstream-2b-interruption-and-mcp.md`.
...
See `../workstreams/workstream-2c-remote-bridge.md`.
...
See `../workstreams/workstream-3-stage-refactor.md`.
...
See `../workstreams/workstream-4-replay-and-bundle.md`.
...
See `../workstreams/workstream-5-legacy-removal.md`.
...
`../workstreams/workstream-1-journal-foundation.md`: write and merge the Phase 1
