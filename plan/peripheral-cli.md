...
> debug-first thesis in `essential-debug-first-runtime.md`. It is the
...
> `cost`, `login`) are deferred to a future `peripheral-cli-extended.md`.
...
EasyCat ships as a library today. No `[project.scripts]`, no
`__main__.py`, no command surface. The library exports ~195 symbols
from `src/easycat/__init__.py`; the first-touch experience is "clone
an example and read it."
...
- **Voice-to-voice / realtime speech-to-speech.** Permanently out
  of scope. See "Chained Only" in `essential-debug-first-runtime.md`.
...
fidelity classes from `workstream-4-replay-and-bundle.md`.
...
lives in `peripheral-cli-extended.md` — separate plan, separate
review.
...
- The library-wrapper commands moved to `peripheral-cli-extended.md`
...
- **LiveKit `lk agent`** (1.5, March 2026): `lk agent init`, hot
  ...
  `peripheral-cli-extended.md`. Scaffolding quality and journal
