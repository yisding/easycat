...
> **Historical implementation checklist.** `src/easycat/session/_turn_runner.py`
...
> **Successors:** none. This is the final phase. The original target
> was ~1,250 `Session` lines; the as-landed code is roughly 1,773
> lines because lifecycle, public APIs, telephony state, action drain,
> and wiring remained on `Session`.
>
...
- [ ] `wc -l src/easycat/session/_session.py` drops by ~620.
  Original target was ~1,250; as landed, `_session.py` is roughly
  1,773 lines.
...
After Phase 5 merges, `src/easycat/session/_session.py` owns lifecycle,
the public event surface, telephony state, action drain, the agent
property, introspection properties, and coordination glue. The original
~1,250-line target proved optimistic; static inspection on 2026-05-21
found `_session.py` at roughly 1,773 lines. Each of the four
collaborators plus TurnRunner is independently testable, journaled, and
replaceable. The decomposition is complete, with residual `Session`
shrinking now treated as follow-up cleanup.
