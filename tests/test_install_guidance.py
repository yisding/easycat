...
def test_template_list_guidance_points_to_catalog_json() -> None:
    ...
    for path in _iter_reader_guidance_files():
        ...
        if "easycat init --list-templates" not in text:
            continue
        if "copyable create/preflight/check/docs/run commands" not in text:
            continue
...
def test_cli_init_examples_name_target_directory() -> None:
    ...
    assert "copyable create/preflight/check/docs/run commands" in normalized_readme
    ...
    assert "copyable create/preflight/check/docs/run commands" in normalized_production_chapter
