...
def test_validation_tasks_v23_current_state_tracks_baseline_comparison_contract() -> None:
    plan = (REPO_ROOT / "plan/validation/tasks.md").read_text(encoding="utf-8")
    section = plan.split("### V2.3 Add Baseline Comparison Helper", 1)[1].split(
        "### V2.4 Add Reliability Sampling To Latency/Stress Runs", 1
    )[0]
    ...
    low_sample_comparison = compare_latency_baseline(
        _latency_artifact_for_comparison(total_ms=1500.0, count=2),
        _latency_artifact_for_comparison(total_ms=1000.0, count=2),
    )
    drift_comparison = compare_latency_baseline(
    ...
    unversioned_baseline = _latency_artifact_for_comparison(total_ms=1000.0)
    unversioned_baseline["baseline"] = {"comparison": "baseline"}
    unversioned_comparison = compare_latency_baseline(current, unversioned_baseline)

    condition = comparison["conditions"][0]
    assert comparison["status"] == "fail"
    assert condition["failure_class"] == "easycat_latency_regression"
    assert low_sample_comparison["conditions"][0]["reason"] == "ineligible_sample_count"
    assert drift_comparison["conditions"][0]["failure_class"] == "provider_api_drift"
    assert drift_comparison["conditions"][0]["reason"] == "condition_mismatch"
    assert unversioned_comparison["conditions"][0]["reason"] == "baseline_version_missing"

    assert "Current verified state:" in section
    for field in comparison:
        assert f"`{field}`" in section
    for field in LatencyComparisonThresholds().to_dict():
        assert f"`{field}`" in section
    for field in (
        "condition_id",
        "baseline_version",
        ...
        "baseline_count",
        "percentile",
        "current_<percentile>_ms",
        "baseline_<percentile>_ms",
        "delta_ms",
        "relative_delta",
    ):
        assert f"`{field}`" in section
    for token in (
        "latency_baseline_comparison",
        "provider_api_drift",
        "condition_mismatch",
        "mixed_condition_signature",
        "baseline_version_missing",
        "ineligible_sample_count",
        "easycat_latency_regression",
        "latency.baseline",
        "latency_baseline",
        "latency_baseline_regression",
    ):
