...
def test_validation_tasks_v34_current_state_tracks_agent_bridge_contracts() -> None:
    plan = (REPO_ROOT / "plan/validation/tasks.md").read_text(encoding="utf-8")
    section = plan.split("### V3.4 Add Agent Bridge Contract Tests", 1)[1].split(
        "### V3.5 Add HTTP/SSE Cassette Proof Of Concept", 1
    )[0]
    normalized_section = " ".join(section.split())
    source = (REPO_ROOT / "tests/contracts/test_agent_bridge_contracts.py").read_text(
        encoding="utf-8"
    )
    rows = [row for row in PROVIDER_SURFACE_CONTRACTS if row.surface == "agent_bridge"]
    expected_providers = {
    ...
    assert {row.provider for row in rows} == expected_providers
    assert {row.contract_path for row in rows} == {
        "tests/contracts/test_agent_bridge_contracts.py"
    }
    assert all(row.adapter for row in rows)
    assert all(row.protocol for row in rows)
    assert all(row.mode for row in rows)
    assert all(row.model_api_version for row in rows)
    assert all(row.cassette_path for row in rows)
    assert all(row.cassette_status in {"required", "deferred"} for row in rows)
    assert all(
        row.live_canary_status in {"required", "deferred", "not_applicable"} for row in rows
    )
    assert all(row.expected_skip_reason for row in rows if row.required_extra)
    assert "pytest.mark.contract" in source
    assert "pytest.mark.agent_bridge" in source
    assert "pytest.mark.surface_agent" in source
    assert 'pytest.mark.provider("offline-fake")' in source
    for token in (
        "text_delta",
        "tool_started",
        "tool_result",
        ...
        "record_unit_exited",
        "record_tool_call",
        "record_framework_handoff",
        "record_state_snapshot",
        "CancellationMode.IMMEDIATE_STOP",
        "record_cancellation_boundary",
        "record_state_committed",
        "run_interruption_journal_protocol",
        "FrameworkStateSnapshot",
        "reset",
    ):
    ...
    for token in ("text_delta", "tool_started", "tool_result", "done", "start", "result"):
        assert f"`{token}`" in section
    for phrase in (
        "missing optional adapter import as an expected skip",
        "cursor enter / exit",
        "framework handoff",
        "swallowed post-commit journal failure",
        "skipped mutation when commit journaling fails",
        "JSON-safe",
        "live framework behavior and normalized framework error taxonomies remain outside",
    ):
