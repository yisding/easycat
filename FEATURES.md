EasyCat is a slim, batteries-included voice bot framework designed to pair with the OpenAI Agents SDK (Python) for agent/tool orchestration, while EasyCat handles audio I/O, VAD/turn-taking, STT/TTS providers, telephony basics (DTMF + voicemail), and noise reduction.
...
Video avatars, vision/image generation, web browsing, “110+ frame types” systems

Multi-LLM provider abstraction (Agents SDK handles the “agent brain”)

Multiple telephony vendors (start with Twilio)
...
Single session object (per call / per websocket client) with:

lifecycle: start(), stop(), shutdown()

cancellation: cancel_turn(), cancel_tts_playback(), reset_state()

Event model (small, streaming-first)
...
Tool calling schemas, handoffs, guardrails, agent memory: those stay in Agents SDK
...
timestamps (optional), confidence (optional), language code (optional)
...
(MVP expectation) Use VAD-driven turn segmentation; submit finalized user turns for transcription.
...
Implementation note (MVP behavior): if Krisp isn’t configured/licensed, EasyCat automatically falls back to Silero without changing the application code.
...
(Design aligns with Agents SDK voice lifecycle event patterns; EasyCat provides interruption behavior even when upstream doesn’t.)
...
Parse and emit DTMF events from Twilio Media Streams dtmf WebSocket messages (supported in bidirectional streams)
...
Emit a single event (dtmf.aggregated) suitable for agent tool use (e.g., “enter account number”).
