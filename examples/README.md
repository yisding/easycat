...
copyable create/preflight/check/fix/docs/run commands for local mic, browser
...
| [twilio_app.py](twilio_app.py) | Twilio Media Streams with per-call EasyCat sessions. | `uv run uvicorn examples.twilio_app:create_app --factory --host 0.0.0.0 --port 8000` | `uv sync --extra openai --extra telephony --extra openai-agents --group dev` | `OPENAI_API_KEY`, `TWILIO_STREAM_URL`; optional `TWILIO_ACCOUNT_SID`, `TWILIO_AUTH_TOKEN`, `TWILIO_VOICE_FROM`, `TWILIO_TWIML_URL`, `TWILIO_STATUS_CALLBACK_URL`, `TWILIO_CALL_API_TOKEN`, `TWILIO_SMS_FROM` |
