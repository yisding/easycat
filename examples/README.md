...
| [telnyx_voice.py](telnyx_voice.py) | App-first Telnyx phone bot via `VoiceApp.run("telnyx")` with Ed25519-verified webhooks and one-time stream tokens. | `uv run python examples/telnyx_voice.py` | `uv sync --extra openai --extra telnyx --extra openai-agents --group dev` | `OPENAI_API_KEY`, `TELNYX_STREAM_URL`, `TELNYX_API_KEY`, `TELNYX_PUBLIC_KEY`; optional `TELNYX_STREAM_TOKEN_SECRET`, `TELNYX_WS_PORT`, `TELNYX_MAX_SESSIONS` |
