...
| [ws_supervisor_server.py](ws_supervisor_server.py) | Browser caller plus passive supervisor listen-in. | `uv run python examples/ws_supervisor_server.py` | `uv sync --extra openai --extra openai-agents --group dev` | `OPENAI_API_KEY`; optional `EASYCAT_SUPERVISOR_TOKEN` |
