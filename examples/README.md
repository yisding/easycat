...
| [reconnecting_ws_client.py](reconnecting_ws_client.py) | Reconnecting client against `ws_server.py`. | `uv run python examples/reconnecting_ws_client.py` | `uv sync --extra openai-agents`; run `ws_server.py` separately | None for client; `OPENAI_API_KEY` for paired `ws_server.py` |
