...
| Example | Use When | Run | Install | Env |
| --- | --- | --- | --- | --- |
| [ws_server.py](ws_server.py) | Multi-session WebSocket server for raw PCM clients. | `uv run python examples/ws_server.py` | `uv sync --extra openai-agents --group dev` | `OPENAI_API_KEY`; optional `EASYCAT_WS_TOKEN` |
| [ws_browser_example.py](ws_browser_example.py) | Browser mic/speaker over WebSocket with local static serving. | `uv run python examples/ws_browser_example.py` | `uv sync --extra openai-agents --group dev` | `OPENAI_API_KEY` |
| [ws_supervisor_server.py](ws_supervisor_server.py) | Browser caller plus passive supervisor listen-in. | `uv run python examples/ws_supervisor_server.py` | `uv sync --extra openai-agents --group dev` | `OPENAI_API_KEY` |
| [reconnecting_ws_client.py](reconnecting_ws_client.py) | Reconnecting client against `ws_server.py`. | `uv run python examples/reconnecting_ws_client.py` | `uv sync --extra openai-agents --group dev`; run `ws_server.py` separately | None for client; `OPENAI_API_KEY` for paired `ws_server.py` |
| [webrtc_server.py](webrtc_server.py) | WebRTC voice chat with bundled browser client. | `uv run python examples/webrtc_server.py` | `uv sync --extra openai-agents --extra webrtc --group dev` | `OPENAI_API_KEY`; optional `TURN_SERVER_URL`, `TURN_USERNAME`, `TURN_CREDENTIAL`, `WEBRTC_EXPOSE_ICE_CREDENTIALS` |
| [webrtc_observability_server.py](webrtc_observability_server.py) | WebRTC plus debugger UI in one browser page. | `uv run python examples/webrtc_observability_server.py` | `uv sync --extra openai-agents --extra webrtc --extra debugger --group dev` | `OPENAI_API_KEY`; optional TURN vars and `WEBRTC_EXPOSE_ICE_CREDENTIALS` |
| [webtransport_server.py](webtransport_server.py) | Multi-client WebTransport server. | `uv run python examples/webtransport_server.py --cert cert.pem --key key.pem` | `uv sync --extra openai-agents --extra webtransport --group dev` | `OPENAI_API_KEY`; local TLS cert/key files |
| [twilio_app.py](twilio_app.py) | Twilio Media Streams with per-call EasyCat sessions. | `uv run uvicorn examples.twilio_app:create_app --factory --host 0.0.0.0 --port 8000` | `uv sync --extra telephony --extra openai-agents --group dev` | `OPENAI_API_KEY`, `TWILIO_STREAM_URL`; optional Twilio call/SMS vars |
